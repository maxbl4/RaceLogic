...
namespace maxbl4.Race.Tests.CheckpointService.Controllers
{
    public class TagControllerTests : IntegrationTestBase
    {
        ...
        public async Task Should_return_stored_tags()
        {
            ...
            tags = await client.GetAsync<List<Tag>>(
                $"{svc.ListenUri}/tag?count=2&start={ts.AddSeconds(50):u}&end={ts.AddSeconds(106):u}");
        ...
        public async Task Should_remove_tags()
        {
            ...
            WithCheckpointStorageService(storageService => { storageService.AppendTag(new Tag {TagId = "1"}); });
