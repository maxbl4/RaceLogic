...
namespace maxbl4.Race.Tests.DataService.Controllers
{
    public class StoreControllerTests : IntegrationTestBase
    {
        ...
        public async Task Should_honor_id_in_uri()
        {
            using var svc = CreateDataService();
            var http = new HttpClient();
            var response = await http.PostBsonAsync($"{svc.ListenUri}/store/Entity/single/5",
                new EntityString {Id = "2", Some = "2"});
            response.StatusCode.Should().Be(HttpStatusCode.Created);
        ...
        public async Task Should_delete_by_id()
        {
            ...
            for (var i = 0; i < 3; i++)
            {
                var response = await http.PostBsonAsync($"{svc.ListenUri}/store/Entity/single",
                    new EntityLong {Some = i.ToString()});
        ...
        public async Task Should_search_with_where()
        {
            ...
            var e = await http.GetBsonAsync<List<Entity>>($"{svc.ListenUri}/store/Entity/search?where=" +
                                                          UrlEncoder.Default.Encode("Int > 3 and Int <= 7"));
            e.Count.Should().Be(4);
        ...
        public async Task Should_search_with_limit()
        {
            ...
            var e = await http.GetBsonAsync<List<Entity>>($"{svc.ListenUri}/store/Entity/search?limit=2&where=" +
                                                          UrlEncoder.Default.Encode("Int > 3 and Int <= 7"));
            e.Count.Should().Be(2);
        }
        ...
        public async Task Should_search_with_order()
        {
            ...
            for (var i = 0; i < 10; i++) e[i].Int.Should().Be(9 - i);
        }
        ...
        public async Task Should_get_count_with_where()
        {
            ...
            var e = await http.GetStringAsync($"{svc.ListenUri}/store/Entity/count?where=" +
                                              UrlEncoder.Default.Encode("Int > 3 and Int <= 7"));
            e.Should().Be("4");
        }
        ...
        private class EntityWithId : IHasId<EntityWithId>
        {
            public string Some { get; set; }
            public string Some2 { get; set; }
            public string Some3 { get; set; }
